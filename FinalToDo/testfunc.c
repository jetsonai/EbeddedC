...
uint8_t TestSetMode = APP_TEST_FIANLTODO;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  else if (TestSetMode == APP_TEST_FIANLTODO) {
...
uint8_t leddata1, leddata2;

void FinalToDoProcess(void)
{
   Queue qdata;
    printf("<FinalToDoProcess Test>\n");
    initQueue(&qdata);
    
    printf("<1. Enqueue fib num>\n");
  for(int i=10; i<= 15; i++)
  {
    uint16_t fibNum = fibNI(i);
    enqueue(&qdata, fibNum);
    printf("Front element is %d\n", peekQ(&qdata));    
  }
  
    printf("<2. Dequeue fib num>\n");
  uint16_t lastDequeueData;
  for(int i=0; i<= 5; i++)
  {
    uint16_t queueData = dequeue(&qdata);
    printf("Dequeued element is %d\n", queueData);  
    lastDequeueData = queueData;
  }
  printf("lastDequeueData is %d (0x%04x) \n", lastDequeueData, lastDequeueData);  
  
    printf("<3. Endian Converting>\n");  
   uint16_t littleEndian16 = lastDequeueData;

    uint16_t bigEndian16 = swap16(littleEndian16);

    printf(" lastDequeueData (little Endian 16b)it: 0x%x\n", littleEndian16);
    printf(" lastDequeueData (big Endian 16bit): 0x%x\n", bigEndian16); 
    
    union EndianConverter converter;

    converter.value16= littleEndian16;
    leddata1 = converter.value8[0];
    leddata2 =  converter.value8[1]; 

    printf(" leddata1 : 0x%02x \n", leddata1);    
    printbin(leddata1);
    printf(" leddata2 : 0x%02x\n", leddata2);    
    printbin(leddata2);
...
{ 
  if( TestSetMode == APP_TEST_FIANLTODO) {
    setAllLed(PIN_SET_OFF); 
    if(TestLed == 1) {
      DoTest1Led();
    } 
